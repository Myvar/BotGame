...
{
    [InstructionHandler(Opcode.SW)]
    public class SouthWestInstruction : Instruction
    {
        public override string Handel(Player p, List<Player> players, Command c)
        {
            for (int i = 0; i < (int) (c.Argument == 0 ? 1 : c.Argument); i++)
            {
                ...
                {
                    return "ERROR North South West is out of bounds";
                }
            ...
            return $"{p.Name} Has Move{(c.Argument == 0 ? "" : "d " + c.Argument + " times")} South West";
        }
    }
}